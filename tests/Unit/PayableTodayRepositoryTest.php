...
class PayableTodayRepositoryTest extends TestCase
{
    /** @test */
    public function total_today_repo_returns_correct_instances()
    {
        $repo = new PayableToday();

        $this->assertInstanceOf(HoursBaseRepository::class, $repo);
    }
    /** @test */
    public function total_today_repo_returns_payable_hours_only()
    {
        $today = now();
        ...
        $repo = (new PayableToday())->getTotal();
    ...
    public function total_today_repo_returns_hours_over_threshold()
    {
        ...
        $repo = (new PayableToday())->overDailyThreshold();
